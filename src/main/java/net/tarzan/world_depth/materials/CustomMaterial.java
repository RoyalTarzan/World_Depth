...
public class CustomMaterial {
    ...
    private final ModArmorMaterials ArmorMaterial;
    ...
    public CustomMaterial(String name, int protection, int durabilityMultiplier, int strength, int attackSpeed, int attackDamage, MobEffect[] mobEffects, Integer[] amplifiers, float knockbackResistance, float toughness, int enchantmentvalue, @Nullable Tier toolTier, Item[] parents, Color color) {
    ...
    public CustomMaterial(String name, int protection, int durabilityMultiplier, int strength, int attackSpeed, int attackDamage, MobEffect[] mobEffects, Integer[] amplifiers, float knockbackResistance, float toughness, int enchantmentvalue, @Nullable Tier toolTier, CustomMaterial[] parents, Color color) {
        this.strength = strength;
        this.attackSpeed = attackSpeed;
        this.attackDamage = attackDamage;
        this.mobEffects = mobEffects;
    ...
    public CustomMaterial(CustomMaterial parent1, CustomMaterial parent2){
        ...
        for (MobEffect effect:parent1.getMobEffects()){
            if (Arrays.stream(parent2.getMobEffects()).toList().contains(effect)){
                ...
                effects.add(effect);
                int amp=(parent1.getAmplifiers()[Arrays.stream(parent1.getMobEffects()).toList().indexOf(effect)]+parent2.getAmplifiers()[index])*5/8;
                amplifiers.add(Math.max(amp, 1));
            } else {
                effects.add(effect);
                int amp=parent1.getAmplifiers()[Arrays.stream(parent1.getMobEffects()).toList().indexOf(effect)]*3/4+1;
                amplifiers.add(Math.max(amp, 1));
            }
        }
        for (MobEffect effect:parent2.getMobEffects()){
            ...
            effects.add(effect);
            int amp=Arrays.stream(parent2.getMobEffects()).toList().indexOf(effect)+1;
            amplifiers.add(Math.max(amp, 1));
        }
        ...
        this.protection=(parent1.getProtection()+parent2.getProtection())*3/5;
        this.durabilityMultiplier= parent1.getDurabilityMultiplier()+parent2.getDurabilityMultiplier();
        this.strength= parent1.getStrength()+parent2.getStrength();
        this.attackSpeed= parent1.getAttackSpeed()+parent2.getAttackSpeed();
        this.attackDamage=parent1.getAttackDamage()+parent2.getAttackDamage();
        this.mobEffects= effects.toArray(new MobEffect[0]);
        this.amplifiers= amplifiers.toArray(new Integer[0]);
        this.knockbackResistance= parent1.getKnockbackResistance()+parent2.getKnockbackResistance();
        this.toughness=parent1.getToughness()+parent2.getToughness();
        this.enchantmentvalue= parent1.getEnchantmentvalue()+parent2.getEnchantmentvalue();
    ...
    public void registerRecipes(@NotNull Consumer<FinishedRecipe> consumer){
        ModRecipeProvider.fullArmor(Item.get(),this.Armour[0].get(),this.Armour[1].get(),this.Armour[2].get(),this.Armour[3].get(),consumer );
        ModRecipeProvider.toolSet(Item.get(),this.Tools[3].get(),this.Tools[4].get(),this.Tools[1].get(),this.Tools[2].get(),this.Tools[0].get(),consumer);
